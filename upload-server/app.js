...
app.get('/', function (req, res) {
  res.send(
    'This server runs <a href="https://github.com/ravi0lii/node-sharex-server">node-sharex-server</a> v' +
      version +
      ' by <a href="https://github.com/ravi0lii">ravi0lii</a>.'
  );
});
...
app.post('/upload', middleware.keyRequired, function (req, res) {
  ...
  if (!req.files || !req.files.file) {
    ...
    getUniqueFilepathDeferred.promise.then(function (payload) {
      logger.info(
        'Uploading file ' + file.name + ' to ' + payload.path + ' (' + req.locals.shortKey + ')'
      );

      // Check file extension (if enabled)
      if (
        config.fileExtensionCheck.enabled &&
        config.fileExtensionCheck.extensionsAllowed.indexOf(fileExtension) == -1
      ) {
        // Invalid file extension
        logger.info(
          'File ' +
            file.name +
            ' has an invalid extension, aborting... (' +
            req.locals.shortKey +
            ')'
        );
        ...
        file.mv(payload.path, function (err) {
          ...
          logger.info(
            'Uploaded file ' + file.name + ' to ' + payload.path + ' (' + req.locals.shortKey + ')'
          );
          response.uploaded(
            res,
            config.staticFileServerUrl + payload.fileName,
            config.serverUrl + '/delete?filename=' + payload.fileName
          );
        });
      }
    });
  ...
  if (!req.query.filename) {
    ...
    fileExists(filePath, function (err, exists) {
      ...
      if (!exists) {
        // File doesnt exists
        logger.info(
          'File ' + fileName + ' doesnt exists, aborting... (' + req.locals.shortKey + ')'
        );
